...
매일 아침 자동으로 **언어별 독립 트랙**으로 글을 생성합니다:

- **한국어 트랙(한국 기준·네이버 검색광고 실측)**: 트렌드(높은 경쟁력) 5 + 에버그린(낮은 경쟁력) 5 = **한국어 10개**
- **영어 트랙(미국 기준·Google Trends/Ads)**: 트렌드 5 + 에버그린 5 = **영어 10개**
- => 두 언어가 서로 **다른 키워드·다른 내용**으로 각각 10개, **하루 총 20개**

생성한 글은 WordPress에 자동 게시(또는 복붙용 저장)하고 웹/아이폰 대시보드와 구글시트로 관리합니다.
...
## 6.7 언어별 트랙 설정 (한국어=한국, 영어=미국)

`config.json`의 `tracks` 배열이 언어별로 **키워드를 따로 산출**합니다. 각 트랙은 서로 독립이라
한국어 글과 영어 글의 주제가 완전히 다릅니다(번역이 아님).

```json
"tracks": [
  { "lang": "ko", "trends_geo": "KR", "metrics": { "provider": "naver", ... } },
  { "lang": "en", "trends_geo": "US", "metrics": { "provider": "trends", ... } }
]
```

- `lang`: 글 언어. `trends_geo`: 트렌드/꾸준함을 측정할 나라(KR, US 등).
- 트랙마다 `metrics.provider`를 다르게 둘 수 있습니다.
  - **한국어**: `naver` (네이버 월검색량 실측, 무료) — 6.6의 발급법 참고.
  - **영어(미국)**: `trends`(무료, 관심도 기반)가 기본. 구글 절대 검색량이 필요하면 `google_ads`로
    바꾸고 `language_id: "1000"`(영어), `geo_target_id: "2840"`(미국)을 채우세요.
    (한국어 google_ads는 `1012`/`2410`)
- `count_each`는 트랙당 각 유형 개수입니다. 5면 언어당 트렌드5+에버그린5=10개.

> 클라우드(GitHub Actions)에서는 시크릿 `LANGS=ko,en` 이면 두 트랙이 자동 구성됩니다.
> 한국어는 `NAVER_*` 시크릿, 영어는 기본 `trends`(키 불필요)로 동작하며,
> 영어도 구글 실측을 원하면 `EN_METRICS_PROVIDER=google_ads` + `GADS_*` 시크릿을 넣으세요.
